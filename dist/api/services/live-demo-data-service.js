...
export class LiveDemoDataService {
    ...
    async getOverview() {
        const [participants, assets, proofs, settlements, compliance, disclosureAction, latestLedgerSequence,] = await Promise.all([
    ...
    async getParticipants() {
        return this.cached("participants", async () => {
            ...
            for (const wallet of Object.values(this.phase0.wallets ?? {})) {
            ...
            const discoveredIds = await Promise.all([...candidateAddresses].map(async (address) => {
                try {
                    return await this.viewContract(this.phase0.contracts.participantRegistry.contractId, "wallet_owner", ["--wallet", address]);
                }
                catch {
                    return null;
                }
            ...
            const participantRecords = await Promise.all([...knownParticipantIds].map(async (participantIdHash) => {
                const record = await this.viewContract(this.phase0.contracts.participantRegistry.contractId, "get_participant", ["--participant_id_hash", participantIdHash]);
                return {
                    participantIdHash,
                    record,
                };
            }));
            const frozenFlags = await Promise.all(participantRecords.map(({ participantIdHash }) => this.viewContract(this.phase0.contracts.complianceControl.contractId, "is_participant_frozen", ["--participant_id_hash", participantIdHash])));
    ...
    async getAssets() {
        return this.cached("assets", async () => {
            ...
            const results = await Promise.all(catalogs.map(async (catalogAsset) => {
                const sacContractId = String(catalogAsset.sacContractId);
                const liveRecord = await this.viewContract(this.phase0.contracts.assetRegistry.contractId, "get_asset", ["--asset", sacContractId]);
                const paused = await this.viewContract(this.phase0.contracts.complianceControl.contractId, "is_asset_paused", ["--asset", sacContractId]);
                return {
    ...
    async getProofScenarios() {
        return this.cached("proofs", async () => {
            const participantDirectory = await this.getParticipantDirectory();
            const events = await this.fetchContractEvents([this.protocol.contracts.proofGateway.contractId], Number(this.protocol.config?.proofGateway?.validFromLedger ?? 0), 200);
            const revokedByReceipt = new Map();
            const latestRecordedByScenario = new Map();
            ...
            const scenarios = await Promise.all([...latestRecordedByScenario.values()].map(async (event) => {
    ...
    async getSettlementSummaries() {
        return this.cached("settlements", async () => {
            const participantDirectory = await this.getParticipantDirectory();
            const assetBySacId = this.getAssetCatalogBySacId();
            const events = await this.fetchContractEvents([this.protocol.contracts.settlementNettingEngine.contractId], this.protocolStartLedger, 100);
            const batchTransfersBySettlementId = new Map();
            const settlements = [];
            for (const event of events) {
            ...
            for (const event of events) {
                if (event.topic === "execution_settled_dvp") {
                    const record = await this.viewContract(this.protocol.contracts.settlementNettingEngine.contractId, "get_execution_settlement", ["--execution_id", String(event.data.execution_id)]);
                    settlements.push({
                        kind: "direct",
                ...
                if (event.topic === "batch_settled") {
                    ...
                    const batch = await this.viewContract(this.protocol.contracts.settlementNettingEngine.contractId, "get_batch", ["--settlement_id", settlementId]);
    ...
    async getOrderFlowSummary() {
        return this.cached("orders", async () => {
            ...
            const events = await this.fetchContractEvents([this.protocol.contracts.orderCommitPool.contractId], this.protocolStartLedger, 100);
            ...
            const execution = await this.viewContract(this.protocol.contracts.orderCommitPool.contractId, "get_execution", ["--execution_id", String(latestMatch.data.execution_id)]);
            const bidOrder = await this.viewContract(this.protocol.contracts.orderCommitPool.contractId, "get_order", ["--order_id", String(execution.bid_order_id)]);
            const askOrder = await this.viewContract(this.protocol.contracts.orderCommitPool.contractId, "get_order", ["--order_id", String(execution.ask_order_id)]);
            const commitByOrderId = new Map();
            for (const event of events) {
    ...
    async getComplianceSnapshot() {
        return this.cached("compliance", async () => {
            const [participants, assets, events, globalPause, operatorModel] = await Promise.all([
                ...
                this.viewContract(this.phase0.contracts.complianceControl.contractId, "is_globally_paused", []),
            ...
            return {
                ...
                assets: await Promise.all(assets.map(async (asset) => ({
                    ...
                    paused: Boolean(await this.viewContract(this.phase0.contracts.complianceControl.contractId, "is_asset_paused", ["--asset", asset.sacContractId])),
    ...
    async getAuditCase(caseId) {
        const normalizedCaseId = this.normalizeHash(caseId);
        const participantDirectory = await this.getParticipantDirectory();
        const auditEvents = await this.fetchContractEvents([this.protocol.contracts.auditDisclosureRegistry.contractId], this.protocolStartLedger, 100);
        const complianceEvents = await this.fetchContractEvents([this.phase0.contracts.complianceControl.contractId], this.protocolStartLedger, 100);
        ...
        const grants = await Promise.all(grantEvents.map(async (event) => ({
            event,
            record: await this.viewContract(this.protocol.contracts.auditDisclosureRegistry.contractId, "get_grant", ["--grant_id", String(event.data.grant_id)]),
        })));
        const accesses = await Promise.all(accessEvents.map(async (event) => ({
            event,
            record: await this.viewContract(this.protocol.contracts.auditDisclosureRegistry.contractId, "get_access_receipt", ["--receipt_id", String(event.data.receipt_id)]),
        })));
        ...
        const matchingGrant = selectedGrant
            ...
            ?? null;
        const matchingFreeze = [...complianceEvents].reverse().find((event) => event.topic === "participant_freeze_set" && this.normalizeHash(event.data.case_id) === effectiveCaseId) ?? null;
    ...
    async getLatestDisclosureAction() {
        const auditEvents = await this.fetchContractEvents([this.protocol.contracts.auditDisclosureRegistry.contractId], this.protocolStartLedger, 100);
        const latestAccess = [...auditEvents].reverse().find((event) => event.topic === "access_recorded");
        if (!latestAccess) {
            return null;
        }
        const receipt = await this.viewContract(this.protocol.contracts.auditDisclosureRegistry.contractId, "get_access_receipt", ["--receipt_id", String(latestAccess.data.receipt_id)]);
        return {
    ...
    async buildLatestComplianceIncident(events, participants, assets) {
        const participantById = new Map(participants.map((participant) => [participant.participantIdHash, participant]));
        const assetById = new Map(assets.map((asset) => [asset.sacContractId, asset]));
        const latest = [...events].reverse().find((event) => ["participant_freeze_set", "asset_pause_set", "global_pause_set"].includes(event.topic));
        if (!latest) {
            return null;
        }
    ...
    async resolveBatchParticipants(settlementId, participantByWallet) {
        try {
            const transfer = await this.viewContract(this.protocol.contracts.settlementNettingEngine.contractId, "get_batch_transfer", ["--settlement_id", settlementId]);
            const wallets = [
    ...
    async getParticipantDirectory() {
        return this.cached("participant-directory", async () => {
            ...
            const ownerPairs = await Promise.all(missingIdentityMeta.map(async (meta) => {
                try {
                    const participantIdHash = await this.viewContract(this.phase0.contracts.participantRegistry.contractId, "wallet_owner", ["--wallet", String(meta.address)]);
                    return [participantIdHash, meta];
                }
                catch {
                    return null;
                }
    ...
    getAssetCatalogBySacId() {
        const catalogs = [
            ...(Array.isArray(this.phase0.assets?.demo) ? this.phase0.assets.demo : []),
            this.phase0.assets?.usdc,
        ].filter(Boolean);
        return new Map(catalogs.map((asset) => [
    ...
    async getLatestLedgerSequence() {
        const response = await this.fetchRpc("getLatestLedger", {});
        return Number(response.sequence);
    }
    async fetchContractEvents(contractIds, startLedger, limit) {
    ...
    async decodeEventValue(base64Value) {
        const decoded = await this.decodeScVal(base64Value);
        return this.toNativeScVal(decoded);
    }
    async decodeScVal(base64Value) {
    ...
    toNativeScVal(decoded) {
        ...
        if ("map" in decoded && Array.isArray(decoded.map)) {
            return Object.fromEntries(decoded.map.map((entry) => [this.toNativeScVal(entry.key), this.toNativeScVal(entry.val)]));
        }
        if ("vec" in decoded && Array.isArray(decoded.vec)) {
            return decoded.vec.map((value) => this.toNativeScVal(value));
        }
    ...
    async viewContract(contractId, functionName, contractArgs) {
    ...
    async fetchHorizonAccount(address) {
    ...
    inferParticipantKey(primaryWallet, participantIdHash) {
        const walletMeta = Object.values(this.phase0.wallets ?? {})
    ...
    cached(key, loader, ttlMs = CACHE_TTL_MS) {
        const now = Date.now();
