...
const PARTICIPANT_ROLE_LABELS: Record<number, string> = {
...
export class DemoDataService {
  ...
  getParticipants(): ParticipantSummary[] {
    ...
    return phase0Participants.map((participant: JsonRecord) => ({
      key: participant.key,
  ...
  getParticipant(participantKey: string): ParticipantSummary | null {
  ...
  getAssets(): AssetSummary[] {
    ...
    const mappedDemoAssets = demoAssets.map((asset: JsonRecord) => ({
      key: asset.key,
  ...
  getProofScenarios(): ProofScenario[] {
    const verifiers = this.protocol.verifiers ?? {};
    const verifierLookup: Record<string, JsonRecord> = {
  ...
  getSettlementSummaries(): SettlementSummary[] {
    const step10 = this.getLatestReport("step10-onchain-");
    const step9 = this.getLatestReport("step9-onchain-");
  ...
  getOrderFlowSummary(): OrderFlowSummary | null {
  ...
  getAuditCase(caseId: string): AuditCaseView {
    const step12 = this.getLatestReport("step12-onchain-");
    const step10 = this.getLatestReport("step10-onchain-");
  ...
  private buildProofScenario(
    key: string,
    proofType: string,
    participantKey: string,
    verifierKey: string,
    status: ProofScenario["status"],
  ): ProofScenario {
    return {
      key,
      proofType,
  ...
  private normalizeProofKey(scenarioKey: string): string {
    if (scenarioKey.includes("collateral")) return "collateralSufficiency";
    if (scenarioKey.includes("unencumbered")) return "unencumberedLot";
    if (scenarioKey.includes("private-match")) return "privateMatch";
    if (scenarioKey.includes("batch")) return "batchNetting";
    if (scenarioKey.includes("claim")) return "entitlementClaim";
    return PROOF_TYPE_ORDER[0];
  }

  private buildLatestComplianceIncident(report: JsonRecord | null): ComplianceIncident | null {
  ...
  private buildLatestDisclosureAction(report: JsonRecord | null): AuditAction | null {
  ...
  private getLatestReport(prefix: string): JsonRecord | null {
