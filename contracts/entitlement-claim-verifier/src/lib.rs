...
const PROOF_TYPE_ENTITLEMENT_CLAIM: u32 = 6;
const STATEMENT_HASH_HI_INDEX: u32 = 7;
const STATEMENT_HASH_LO_INDEX: u32 = 8;
const MIN_PUBLIC_SIGNAL_COUNT: u32 = 9;
...
impl VerificationKey {
    pub fn to_bytes(&self, env: &Env) -> Bytes {
        ...
        bytes.append(&Bytes::from_array(env, &self.alpha.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &self.beta.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &self.gamma.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &self.delta.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &(self.ic.len() as u32).to_be_bytes()));
        for point in self.ic.iter() {
            bytes.append(&Bytes::from_array(env, &point.to_bytes().to_array()));
        }
        bytes
    }
...
impl Proof {
    pub fn to_bytes(&self, env: &Env) -> Bytes {
        let mut bytes = Bytes::new(env);
        bytes.append(&Bytes::from_array(env, &self.a.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &self.b.to_bytes().to_array()));
        bytes.append(&Bytes::from_array(env, &self.c.to_bytes().to_array()));
        bytes
    }
...
pub fn encode_proof_payload(env: &Env, proof: &Proof, public_signals: &Vec<Fr>) -> Bytes {
    let mut bytes = proof.to_bytes(env);
    bytes.append(&Bytes::from_array(env, &(public_signals.len() as u32).to_be_bytes()));
    for signal in public_signals.iter() {
        bytes.append(&Bytes::from_array(env, &signal.to_bytes().to_array()));
    }
    bytes
}
...
#[contract]
pub struct EntitlementClaimVerifier;

#[contractimpl]
impl EntitlementClaimVerifier {
    ...
    pub fn verify(env: Env, proof_type: u32, statement_hash: BytesN<32>, proof: Bytes) -> bool {
        if proof_type != PROOF_TYPE_ENTITLEMENT_CLAIM {
            return false;
        }
