...
fn encode_proof_payload(env: &Env, proof: &Proof, public_signals: &Vec<Fr>) -> Bytes {
    let mut bytes = Bytes::new(env);
    bytes.append(&Bytes::from_array(env, &proof.a.to_bytes().to_array()));
    bytes.append(&Bytes::from_array(env, &proof.b.to_bytes().to_array()));
    bytes.append(&Bytes::from_array(env, &proof.c.to_bytes().to_array()));
...
fn payload_roundtrip_preserves_statement_hash_binding() {
    ...
    let statement_hash = BytesN::from_array(
        ...
        &[
            ...
            0xee, 0xff, 0x00, 0x10, 0x20, 0x30, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xa0,
...
fn verifies_real_generated_bundle() {
    let output = Command::new("node")
        ...
        .arg("scripts/generate-collateral-sufficiency-proof.mjs")
