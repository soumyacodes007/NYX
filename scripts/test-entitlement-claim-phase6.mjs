...
console.log("Preparing circuit build and Groth16 setup once for all Phase 6 tests...");
...
console.log("\nTest 2: tampered quantity should fail witness generation");
{
  const { fullPath } = await writeFixtureInput("test2.bad-qty.input.json", (input) => {
    input.entitlementQuantity = (BigInt(input.entitlementQuantity) + 1n).toString();
    return input;
  });
  const witnessPath = path.join(artifactDir, "test2.bad-qty.wtns");
...
console.log("\nTest 3: tampered public signals should fail verification");
{
  ...
  const publicSignals = await readJson(publicPath);
  publicSignals[4] = (BigInt(publicSignals[4]) + 1n).toString();
...
console.log("\nAll three Phase 6 real tests passed.");
