...
if (!statementHashHex) {
  throw new Error(
    "usage: node scripts/generate-collateral-sufficiency-proof.mjs <statement-hash-hex> [name] [options-json]",
  );
}
...
const bundle = {
  statementHashHex,
  participantIdHashHex: toPrefixedHex(toHex32(fixtureBundle.participantIdHash)),
  portfolioCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.portfolioCommitment)),
  proofNonceHex: toPrefixedHex(toHex32(fixtureBundle.proofNonce)),
  requiredMargin: fixtureBundle.requiredMargin.toString(),
  totalAdjustedValue: fixtureBundle.totalAdjustedValue.toString(),
...
process.stdout.write(
  `__PHASE2_BUNDLE__${JSON.stringify(
    stringifyDeep({
      proofPath,
      publicPath,
      verificationKeyPath,
