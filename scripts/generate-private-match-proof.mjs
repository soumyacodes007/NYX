...
if (!statementHashHex) {
  throw new Error(
    "usage: node scripts/generate-private-match-proof.mjs <statement-hash-hex> [name] [options-json]",
  );
}
...
const bundle = {
  statementHashHex,
  bidOrderCommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[0])),
  askOrderCommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[1])),
  instrumentIdHashHex: toPrefixedHex(toHex32(expected.publicSignals[2])),
  executionCommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[3])),
...
process.stdout.write(
  `__PHASE4_BUNDLE__${JSON.stringify(
    stringifyDeep({
      proofPath,
      publicPath,
      verificationKeyPath,
