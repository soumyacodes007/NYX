...
if (!statementHashHex) {
  throw new Error(
    "usage: node scripts/generate-batch-netting-proof.mjs <statement-hash-hex> [name] [options-json]",
  );
}

const fixtureOptions = JSON.parse(rawOptions);
...
const { bundle: fixtureBundle } = expected;
const bundle = {
  statementHashHex,
  batchIdHex: toPrefixedHex(toHex32(fixtureBundle.batchIdHash)),
  instrumentIdHashHex: toPrefixedHex(toHex32(fixtureBundle.instrumentIdHash)),
  executionACommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[0])),
  executionBCommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[1])),
  settlementCommitmentHex: toPrefixedHex(toHex32(expected.publicSignals[2])),
  tradeNullifierAHex: toPrefixedHex(toHex32(expected.publicSignals[3])),
  tradeNullifierBHex: toPrefixedHex(toHex32(expected.publicSignals[4])),
  netVectorHashHex: toPrefixedHex(toHex32(fixtureBundle.netVectorHash)),
  ...
  tradeA: {
    bidOrderCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.tradeA.bidOrderCommitment)),
    askOrderCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.tradeA.askOrderCommitment)),
  },
  tradeB: {
    bidOrderCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.tradeB.bidOrderCommitment)),
    askOrderCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.tradeB.askOrderCommitment)),
  },
  fixtureOptions,
};

process.stdout.write(
  `__PHASE5_BUNDLE__${JSON.stringify(
    stringifyDeep({
      proofPath,
      publicPath,
      verificationKeyPath,
