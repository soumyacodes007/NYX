...
console.log("Preparing circuit build and Groth16 setup once for all Phase 5 tests...");
...
console.log("\nTest 2: inconsistent slot net cash should fail witness generation");
{
  const { fullPath } = await writeFixtureInput("test2.bad-nets.input.json", (input) => {
    input.slotNetCash[0] = (BigInt(input.slotNetCash[0]) + 1n).toString();
    return input;
  });
  const witnessPath = path.join(artifactDir, "test2.bad-nets.wtns");
...
console.log("\nTest 3: tampered public signals should fail verification");
{
  ...
  const publicSignals = await readJson(publicPath);
  publicSignals[2] = (BigInt(publicSignals[2]) + 1n).toString();
...
console.log("\nAll three Phase 5 real tests passed.");
