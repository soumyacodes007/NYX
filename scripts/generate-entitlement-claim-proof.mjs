...
if (!statementHashHex) {
  throw new Error(
    "usage: node scripts/generate-entitlement-claim-proof.mjs <statement-hash-hex> [name] [options-json]",
  );
}
...
const bundle = {
  statementHashHex,
  assetIdHashHex: toPrefixedHex(toHex32(fixtureBundle.assetIdHash)),
  eventIdHashHex: toPrefixedHex(toHex32(fixtureBundle.eventIdHash)),
  eventRootHex: toPrefixedHex(toHex32(fixtureBundle.eventRoot)),
  claimCommitmentHex: toPrefixedHex(toHex32(fixtureBundle.claimCommitment)),
  claimNullifierHex: toPrefixedHex(toHex32(fixtureBundle.claimNullifier)),
  claimAmount: fixtureBundle.claimAmount.toString(),
  entitlementQuantity: fixtureBundle.entitlementQuantity.toString(),
...
process.stdout.write(
  `__PHASE6_BUNDLE__${JSON.stringify(
    stringifyDeep({
      proofPath,
      publicPath,
      verificationKeyPath,
